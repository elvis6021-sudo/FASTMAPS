...
// FASE 2 (2026-07-27): clasificación FINA por clase OSM. Separa motorway/trunk/primary/secondary/
// tertiary en capas propias -- cada una con su color y "casing" reales de OSM-Carto -- en vez de
// aplastar todo en 3 niveles. Así el mapa distingue autopista (rosa), troncal (naranja), primaria
// (naranja claro), secundaria (amarillo) y terciaria (blanco), como openstreetmap.org.
const HIGHWAY_CLASS = {
...
// Clases en orden de importancia (residential = resto). El orquestador genera un `.TAB` por clase.
const ROAD_CLASSES = ['motorway', 'trunk', 'primary', 'secondary', 'tertiary', 'residential'];

function classForWay(way) {
  return HIGHWAY_CLASS[way.tags.highway] || 'residential'; // resto = calles locales (AllStreets)
}
...
function writeStreetsTab(parsed, outTabPath, layerName, opts = {}) {
  ...
  for (const way of parsed.ways) {
    if (opts.roadClass !== undefined && classForWay(way) !== opts.roadClass) continue; // pertenece a otra CLASE de vía
    else if (opts.tier !== undefined && tierForWay(way) !== opts.tier) continue; // (legacy) pertenece a otro nivel
    ...
    let line = new gdal.LineString();
    for (const [lon, lat] of coords) line.points.add(lon, lat);
    // FASE 5 (ligereza): simplificación opcional Douglas-Peucker, en GRADOS. Reduce vértices del
    // fondo VISUAL sin tocar el ruteo (`.lmb` es aparte). Off por default -- pasar
    // `opts.simplifyTolerance` (ej. 0.00003 ≈ ~3m) para `.TAB` más livianos.
    if (opts.simplifyTolerance) { const s = line.simplify(opts.simplifyTolerance); if (s && !s.isEmpty()) line = s; }
...
module.exports = { writeStreetsTab, classForWay, tierForWay, ROAD_CLASSES, HIGHWAY_CLASS };
